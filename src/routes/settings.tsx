...
function SettingsPage() {
  ...
  const handleSaveSheets = () => {
    ...
    useMobileStore.getState().updateSheetsConfig({ url: trimmed, enabled: true });
  ...
  const handleDisconnectDatabase = () => {
    ...
    useMobileStore.getState().updateSheetsConfig({ url: "", enabled: false, lastSync: undefined });
    setTestResult(null);
  ...
  const handleSyncNow = async () => {
    ...
    if (urlInput.trim() && urlInput.trim() !== sheetsConfig.url) {
      updateSheetsConfig({ url: urlInput.trim() });
    }
    setSyncing(true);
